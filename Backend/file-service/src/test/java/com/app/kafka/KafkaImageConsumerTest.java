...
class KafkaImageConsumerTest {
	...
	void consume_deletion_of_track() {
		kafkaTemplate.send("trackDeletionTopic", new ImageDeletionMessage(NAME_OF_TEST_FILE))
		.thenRun(() -> assertThat(Paths.get(testImagePath, NAME_OF_TEST_FILE)).doesNotExist());
	}
	...
	@SneakyThrows
	void deleteFile() {
		Files.deleteIfExists(Paths.get(testImagePath, NAME_OF_TEST_FILE));
	}
