...
public class PlaylistService {
    ...
    public Mono<ResponseEntity<?>> createPlaylist(CreateOrUpdatePlaylist dto, Integer userId) {
        if (dto.getCover() != null) {
            ...
            return dto.getCover().transferTo(file)
                    ...
                    .flatMap(t -> client.savePlaylistCover(t, file))
    ...
    public Mono<Void> updatePlaylist(CreateOrUpdatePlaylist dto, Integer id, Integer userId) {
        if (dto.getCover() != null) {
            ...
            return dto.getCover().transferTo(file)
                    ...
                    .switchIfEmpty(Mono.error(() -> new ResponseStatusException(HttpStatus.FORBIDDEN)))
                    .flatMap(t -> client.savePlaylistCover(t, file))
                    .flatMap(t -> checkCreatedByAndUpdatePlaylist(t, dto, userId))
                    .flatMap(playlistRepository::save)
                    .doFinally(t -> file.delete())
                    .then();
    ...
    private Mono<Playlist> checkCreatedByAndUpdatePlaylist(Playlist playlist, CreateOrUpdatePlaylist dto, Integer userId) {
        return Mono.just(playlist)
                .filter(t -> t.getCreatedBy() == userId)
                .switchIfEmpty(Mono.error(() -> new ResponseStatusException(HttpStatus.FORBIDDEN)))
                .flatMap(t -> updatePlaylistFields(t, dto, userId));
    }

    private Mono<Playlist> updatePlaylistFields(Playlist playlist, CreateOrUpdatePlaylist dto, Integer userId) {
